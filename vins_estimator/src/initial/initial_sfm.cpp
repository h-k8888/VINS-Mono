...
{
	vector<cv::Point2f> pts_2_vector;//像素点集合
	vector<cv::Point3f> pts_3_vector;//3D点集合
	for (int j = 0; j < feature_num; j++)
	{
		...
		{
			if (sfm_f[j].observation[k].first == i)//特征在第i图像帧
			{
			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	q[l].z() = 0;
	T[l].setZero();
	q[frame_num - 1] = q[l] * Quaterniond(relative_R);//当前帧到参考帧q
	...
	double c_rotation[frame_num][4];//double array for ceres
	double c_translation[frame_num][3];
	Eigen::Matrix<double, 3, 4> Pose[frame_num];//位姿
	...
	for (int j = 0; j < feature_num; j++)
	{
		...
		if ((int)sfm_f[j].observation.size() >= 2)
		{
			...
			int frame_0 = sfm_f[j].observation[0].first;//特征出现的第一帧
			point0 = sfm_f[j].observation[0].second;
			int frame_1 = sfm_f[j].observation.back().first;//特征出现的最后一帧
...
*/
	...
	for (int i = 0; i < frame_num; i++)
	{
		...
		if (i == l)
		{
			problem.SetParameterBlockConstant(c_rotation[i]);//优化过程不改变参考帧l的姿态
		}
		if (i == l || i == frame_num - 1)
		{
			problem.SetParameterBlockConstant(c_translation[i]);//优化过程不改变参考帧l和最新帧的位移
		}
	}
